...
(() => {
	...
	// Exportar funções CNES globalmente para uso no HTML inline
	...
	// --- Função global para inserir texto no editor ---
	// Definida fora do callback 'load' para estar disponível imediatamente
	window.insertAtCursor = function (textOrCode) {
		if (!textOrCode || isBlocked) return;

		// Buscar editor dinamicamente para garantir referência atualizada
		const editor = editorDiv || document.getElementById('textArea');
		if (!editor) {
			console.warn('Editor #textArea não encontrado');
			return;
		}

		// Garantir que hotstringsRef esteja disponível
		ensureHotstrings();
		...
		// Tentar ajustar fonte se a função existir
	...
	window.addEventListener('load', () => {
		setTimeout(() => {
			...
			// window.insertAtCursor já definida globalmente fora do callback load
